...
public class JsonTools {
	 ...
	 */
	public static org.openfoodfacts.utils.Tuple<Long, Long> findBarcodeInJsonStream(String barcodeToFind, InputStream in) throws UnsupportedEncodingException, IOException {
		org.openfoodfacts.utils.Tuple<Long, Long> positionBarcode;
		long pos = 0;
		long total = 0;
		...
		positionBarcode = new org.openfoodfacts.utils.Tuple<>(pos, total);
	...
	public static org.openfoodfacts.utils.Tuple< List<IProduct>, List<IProduct>> extractOneCellMatrix(String code_h, String code_w, String fullpath_all_products, int width, String fullpath_updated_products, int height) {
		...
		try {
			...
			org.openfoodfacts.utils.Tuple<Long, List<IProduct>> h_tuple = JsonTools.extractOneCellMatrixInOneDimension(istr_updated_products, code_h, height);
			if (h_tuple == null) {
				h_products = null;
			} else {
		...
		try {
			...
			org.openfoodfacts.utils.Tuple<Long, List<IProduct>> w_tuple = JsonTools.extractOneCellMatrixInOneDimension(istr_all_products, code_w, width);
			if (w_tuple == null) {
				w_products = null;
			} else {
		...
		org.openfoodfacts.utils.Tuple<List<IProduct>, List<IProduct>> matrixProducts = new org.openfoodfacts.utils.Tuple<>(w_products, h_products);
	 ...
	 */
	public static org.openfoodfacts.utils.Tuple<Long, List<IProduct>> extractOneCellMatrixInOneDimension(InputStream in, String code, int length) {
		...
		org.openfoodfacts.utils.Tuple<Long, List<IProduct>> result = new Tuple<>(nb_read, products);
